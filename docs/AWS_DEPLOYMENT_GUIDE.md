...
**重要:** NAT Gatewayは**作成しません**。CI/CDでビルドすることで月額$45のコスト削減が可能です。

### 3.1 AWS Management Console にログイン

#### ステップ1: AWS にログイン

1. ブラウザで https://console.aws.amazon.com/ を開く
2. **ルートユーザーまたはIAMユーザー**でログイン
3. リージョンが **ap-northeast-1（東京）** になっていることを確認
   - 画面右上のリージョン選択で「アジアパシフィック（東京）ap-northeast-1」を選択

### 3.2 VPC ダッシュボードを開く

#### ステップ2: VPC サービスに移動

1. 画面上部の **検索バー**に「**VPC**」と入力
2. 検索結果から「**VPC**」をクリック
3. VPC ダッシュボードが開きます
...
1. 左サイドメニューから「**お使いのVPC**」をクリック
2. 画面右上の「**VPCを作成**」ボタン（オレンジ色）をクリック
...
1. 「**VPC など**」を選択（デフォルト）
   - これにより、VPC、サブネット、ルートテーブル、インターネットゲートウェイが自動作成されます

**「名前タグの自動生成」セクション:**

2. 「**名前タグの自動生成**」の入力欄に「**testcasedb**」と入力
   - プレビューが表示され、リソース名が「testcasedb-vpc」「testcasedb-subnet-public1-ap-northeast-1a」等になることが確認できます

**「IPv4 CIDR ブロック」セクション:**

3. 「**IPv4 CIDR ブロック**」は「**10.0.0.0/16**」のまま（デフォルト）
4. 「**IPv6 CIDR ブロック**」は「**IPv6 CIDR ブロックなし**」を選択

**「テナンシー」セクション:**

5. 「**テナンシー**」は「**デフォルト**」を選択

**「アベイラビリティゾーン (AZ) の数」セクション:**

6. 「**アベイラビリティゾーン (AZ) の数**」で「**2**」を選択
   - 高可用性のため、2つのAZを使用します

**「パブリックサブネットの数」セクション:**

7. 「**パブリックサブネットの数**」で「**2**」を選択
   - ALBを配置するためのパブリックサブネットです

**「プライベートサブネットの数」セクション:**

8. 「**プライベートサブネットの数**」で「**2**」を選択
   - ECS、RDS を配置するためのプライベートサブネットです

**「NAT ゲートウェイ ($)」セクション（重要）:**

9. 🔴 **「NAT ゲートウェイ」で「なし」を選択** 🔴
   - ⚠️ 「1 AZ 内」は選択しないでください（月額$45のコストがかかります）
   - CI/CDでビルドするため、NAT Gatewayは不要です（3.4節参照）

**「VPC エンドポイント」セクション:**

10. 「**VPC エンドポイント**」で「**S3 ゲートウェイ**」にチェック ✅
    - S3アクセス用の無料エンドポイントです

**「DNS オプション」セクション:**

11. 「**DNS ホスト名を有効化**」にチェック ✅
12. 「**DNS 解決を有効化**」にチェック ✅（デフォルトでチェック済み）

#### ステップ5: 設定の確認

画面右側の「**プレビュー**」セクションで、以下が表示されていることを確認：
...
- ルートテーブル: 2
- インターネットゲートウェイ: 1
- NAT ゲートウェイ: 0  ← ✅ 0であることを確認！
- VPC エンドポイント: 1 (S3)
```

#### ステップ6: VPC を作成

1. 画面下部の「**VPCを作成**」ボタン（オレンジ色）をクリック
2. 作成中の画面が表示されます（約2-3分かかります）
3. 「**VPCを正常に作成しました**」というメッセージが表示されたら完了
...
1. 左サイドメニューから「**お使いのVPC**」をクリック
2. 「**testcasedb-vpc**」が作成されていることを確認
   - VPC ID（例: vpc-0123456789abcdef0）をメモしておきます

#### ステップ8: サブネットの確認

1. 左サイドメニューから「**サブネット**」をクリック
2. 以下の4つのサブネットが作成されていることを確認：
...
3. 各サブネットIDをメモしておきます（後の手順で使用）

#### ステップ9: インターネットゲートウェイの確認

1. 左サイドメニューから「**インターネットゲートウェイ**」をクリック
2. 「**testcasedb-igw**」が作成され、VPCにアタッチされていることを確認

#### ステップ10: ルートテーブルの確認

1. 左サイドメニューから「**ルートテーブル**」をクリック
2. 以下の2つのルートテーブルが作成されていることを確認：

**パブリックサブネット用ルートテーブル:**
- 名前: `testcasedb-rtb-public`
- ルート: `0.0.0.0/0` → インターネットゲートウェイ

**プライベートサブネット用ルートテーブル:**
- 名前: `testcasedb-rtb-private1-ap-northeast-1a`
- 名前: `testcasedb-rtb-private2-ap-northeast-1c`
- ルート: ローカル通信のみ（NAT Gatewayへのルートはなし）✅

#### ステップ11: VPC エンドポイントの確認

1. 左サイドメニューから「**エンドポイント**」をクリック
2. S3 Gateway エンドポイントが作成されていることを確認
...
| 項目 | 確認内容 | 状態 |
|------|---------|------|
| ✅ VPC | testcasedb-vpc が作成されている | ☐ |
| ✅ パブリックサブネット | 2つ作成されている (AZ: 1a, 1c) | ☐ |
| ✅ プライベートサブネット | 2つ作成されている (AZ: 1a, 1c) | ☐ |
| ✅ インターネットゲートウェイ | VPCにアタッチされている | ☐ |
| ✅ S3 エンドポイント | Gateway型が作成されている | ☐ |
| ✅ NAT Gateway | **0個（作成されていない）** ✅ | ☐ |

**重要:** NAT Gatewayが0個であることを必ず確認してください。もし作成されている場合は、3.4.6節の削除手順を参照してください。
...
### 3.7 追加のVPC Endpointの作成（ECR等）

NAT Gatewayを作成しない構成では、ECRからDockerイメージを取得するためにVPC Endpointが必要です。

#### 3.7.1 必要なVPC Endpoint一覧

| エンドポイント | タイプ | 必須度 | 月額コスト | 用途 |
|-------------|--------|--------|-----------|------|
| **ECR API** | Interface | ✅ 必須 | ~$7 | Docker操作API |
| **ECR DKR** | Interface | ✅ 必須 | ~$7 | Dockerイメージ取得 |
| **Secrets Manager** | Interface | ⭐ 推奨 | ~$7 | 認証情報取得 |
| **CloudWatch Logs** | Interface | ⭐ 推奨 | ~$7 | ログ出力 |
| **S3** | Gateway | ✅ 作成済み | 無料 | ファイルストレージ |

**合計月額:** 約$21-28（Interface型エンドポイント3-4個）

#### 3.7.2 ECR API Endpoint の作成

##### ステップ1: エンドポイント作成画面を開く

1. VPC ダッシュボードの左サイドメニューから「**エンドポイント**」をクリック
2. 画面右上の「**エンドポイントを作成**」ボタン（オレンジ色）をクリック

##### ステップ2: 基本設定

1. **名前タグ** - オプション: `testcasedb-ecr-api-endpoint` と入力
2. **サービスカテゴリ**: `AWS のサービス` を選択（デフォルト）

##### ステップ3: サービスを選択

1. **サービス**の検索欄に `ecr.api` と入力
2. `com.amazonaws.ap-northeast-1.ecr.api` を選択
   - タイプ: Interface
   - 所有者: amazon

##### ステップ4: VPC と AZ を選択

1. **VPC**: `testcasedb-vpc` を選択
2. **アベイラビリティゾーン**: 2つのAZにチェック
   - `ap-northeast-1a` にチェック
   - `ap-northeast-1c` にチェック
3. **サブネットID**:
   - AZ `ap-northeast-1a`: `testcasedb-subnet-private1-ap-northeast-1a` を選択
   - AZ `ap-northeast-1c`: `testcasedb-subnet-private2-ap-northeast-1c` を選択
4. **IPv4 アドレスタイプ**: `IPv4` を選択

##### ステップ5: セキュリティグループを選択

1. **セキュリティグループ**で、デフォルトのセキュリティグループのチェックを**外す**
2. 「**新しいセキュリティグループを作成**」リンクをクリック（新しいタブが開きます）

**新しいセキュリティグループの作成:**

3. 新しいタブで以下を入力:
   - **セキュリティグループ名**: `testcasedb-vpc-endpoint-sg`
   ...
   - **インバウンドルール**:
     ...
     - 説明: `Allow from VPC`
   - 「**セキュリティグループを作成**」をクリック
4. 元のタブに戻り、右上の🔄更新ボタンをクリック
5. `testcasedb-vpc-endpoint-sg` を選択

##### ステップ6: ポリシーとタグ

1. **ポリシー**: `フルアクセス` のまま（デフォルト）
2. **タグ** - オプション: 追加しない

##### ステップ7: エンドポイントを作成

1. 画面下部の「**エンドポイントを作成**」ボタンをクリック
2. ステータスが「**利用可能**」になるまで待ちます（約2-3分）

#### 3.7.3 ECR DKR Endpoint の作成

同様の手順でECR DKRエンドポイントを作成します。

1. 「**エンドポイントを作成**」をクリック
2. **名前タグ**: `testcasedb-ecr-dkr-endpoint`
3. **サービス**: `com.amazonaws.ap-northeast-1.ecr.dkr` を選択
4. **VPC**: `testcasedb-vpc`
5. **サブネット**: プライベートサブネット2つを選択（AZ: 1a, 1c）
6. **セキュリティグループ**: `testcasedb-vpc-endpoint-sg` を選択
7. 「**エンドポイントを作成**」をクリック

#### 3.7.4 Secrets Manager Endpoint の作成（推奨）

1. 「**エンドポイントを作成**」をクリック
2. **名前タグ**: `testcasedb-secretsmanager-endpoint`
3. **サービス**: `com.amazonaws.ap-northeast-1.secretsmanager` を選択
4. **VPC**: `testcasedb-vpc`
5. **サブネット**: プライベートサブネット2つを選択
6. **セキュリティグループ**: `testcasedb-vpc-endpoint-sg`
7. 「**エンドポイントを作成**」をクリック

#### 3.7.5 CloudWatch Logs Endpoint の作成（推奨）

1. 「**エンドポイントを作成**」をクリック
2. **名前タグ**: `testcasedb-logs-endpoint`
3. **サービス**: `com.amazonaws.ap-northeast-1.logs` を選択
4. **VPC**: `testcasedb-vpc`
5. **サブネット**: プライベートサブネット2つを選択
6. **セキュリティグループ**: `testcasedb-vpc-endpoint-sg`
7. 「**エンドポイントを作成**」をクリック

#### 3.7.6 VPC Endpoint 作成完了チェックリスト

エンドポイント一覧画面で、以下が「利用可能」になっていることを確認:

| エンドポイント | サービス名 | タイプ | 状態 |
|-------------|-----------|--------|------|
| ✅ S3 Gateway | com.amazonaws.ap-northeast-1.s3 | Gateway | 利用可能 |
| ✅ ECR API | com.amazonaws.ap-northeast-1.ecr.api | Interface | 利用可能 |
| ✅ ECR DKR | com.amazonaws.ap-northeast-1.ecr.dkr | Interface | Interface |
| ✅ Secrets Manager | com.amazonaws.ap-northeast-1.secretsmanager | Interface | 利用可能 |
| ✅ CloudWatch Logs | com.amazonaws.ap-northeast-1.logs | Interface | 利用可能 |

**重要:** すべてのエンドポイントのステータスが「**利用可能**」になるまで待ってから、次のステップに進んでください（各2-3分）。

---

## 4. セキュリティグループの作成

4つのセキュリティグループを作成します。順番に作成していきましょう。

### 4.1 ALB 用セキュリティグループ（プロキシIP 制限付き）

#### ステップ1: セキュリティグループ作成画面を開く

1. VPC ダッシュボードの左サイドメニューから「**セキュリティグループ**」をクリック
2. 画面右上の「**セキュリティグループを作成**」ボタン（オレンジ色）をクリック

#### ステップ2: 基本情報を入力

1. **セキュリティグループ名**: `testcasedb-alb-sg` と入力
2. **説明**: `Security group for Internet-facing ALB with proxy IP restriction` と入力
3. **VPC**: プルダウンメニューから `testcasedb-vpc` を選択

#### ステップ3: インバウンドルールを設定

1. 「**インバウンドルール**」セクションの「**ルールを追加**」ボタンをクリック

**ルール1: プロキシサーバーからのHTTPS**

2. **タイプ**: `HTTPS` を選択（ポート443が自動入力される）
3. **ソース**: `カスタム` を選択
4. **CIDR**: `203.0.113.50/32` と入力（⚠️ 実際の客先プロキシIPに置き換えてください）
5. **説明**: `Proxy Server` と入力

**ルール2: 管理者用緊急アクセス（オプション）**

6. もう一度「**ルールを追加**」をクリック
7. **タイプ**: `HTTPS`
8. **ソース**: `カスタム`
9. **CIDR**: `203.0.113.100/32` と入力（⚠️ 実際の管理者IPに置き換えてください）
10. **説明**: `Admin Emergency Access` と入力

> **重要**: `203.0.113.50` はサンプルIPです。必ず実際の客先プロキシサーバーの固定IPアドレスに置き換えてください。

#### ステップ4: アウトバウンドルールを確認

1. 「**アウトバウンドルール**」セクション
2. デフォルトで「すべてのトラフィック → 0.0.0.0/0」が設定されています（変更不要）

#### ステップ5: セキュリティグループを作成

1. 画面下部の「**セキュリティグループを作成**」ボタン（オレンジ色）をクリック
2. 「**testcasedb-alb-sg を正常に作成しました**」というメッセージが表示されたら完了
3. セキュリティグループIDをメモしておきます（例: sg-0123456789abcdef0）

---

### 4.2 ECS 用セキュリティグループ

#### ステップ1: 新しいセキュリティグループを作成

1. セキュリティグループ一覧画面で「**セキュリティグループを作成**」ボタンをクリック

#### ステップ2: 基本情報を入力

1. **セキュリティグループ名**: `testcasedb-ecs-sg` と入力
2. **説明**: `Security group for ECS Fargate tasks` と入力
3. **VPC**: `testcasedb-vpc` を選択
...
3. **ポート範囲**: `3000` と入力
4. **ソース**: `カスタム` を選択
5. 入力欄に「sg-」と入力すると、セキュリティグループの候補が表示されます
6. `testcasedb-alb-sg` を選択
7. **説明**: `Allow from ALB` と入力

#### ステップ4: アウトバウンドルールを確認

デフォルトで「すべてのトラフィック → 0.0.0.0/0」が設定されています（変更不要）
...
1. **セキュリティグループ名**: `testcasedb-rds-sg` と入力
2. **説明**: `Security group for RDS PostgreSQL` と入力
3. **VPC**: `testcasedb-vpc` を選択
...
1. 「**ルールを追加**」をクリック
2. **タイプ**: `PostgreSQL` を選択（ポート5432が自動入力される）
3. **ソース**: `カスタム` を選択
4. `testcasedb-ecs-sg` を選択
5. **説明**: `Allow from ECS` と入力
...
7. **タイプ**: `PostgreSQL`
8. **ソース**: `カスタム`
9. `testcasedb-batch-sg` を選択（⚠️ まだ作成していないので、後で追加します）
10. **説明**: `Allow from Batch` と入力

> **注**: testcasedb-batch-sgはまだ作成していないので、このルールは後で追加します（4.4完了後）

#### ステップ4: セキュリティグループを作成

Batchからのルールを除いて、まず作成します。「**セキュリティグループを作成**」ボタンをクリック
...
1. **セキュリティグループ名**: `testcasedb-batch-sg` と入力
2. **説明**: `Security group for AWS Batch` と入力
3. **VPC**: `testcasedb-vpc` を選択

#### ステップ3: インバウンドルールを設定

インバウンドルールは設定しません（外部からのアクセス不要）
...
### 4.5 RDS セキュリティグループにBatchルールを追加

Batchセキュリティグループが作成されたので、RDSセキュリティグループに戻ってルールを追加します。

#### ステップ1: RDSセキュリティグループを開く

1. セキュリティグループ一覧から `testcasedb-rds-sg` をクリック
2. 下部の「**インバウンドルール**」タブをクリック
3. 「**インバウンドルールを編集**」ボタンをクリック

#### ステップ2: Batchからのルールを追加

1. 「**ルールを追加**」をクリック
2. **タイプ**: `PostgreSQL`
3. **ソース**: `カスタム`
4. `testcasedb-batch-sg` を選択
5. **説明**: `Allow from Batch` と入力
6. 「**ルールを保存**」ボタンをクリック

---

### 4.6 セキュリティグループ作成完了チェックリスト

| セキュリティグループ | 用途 | インバウンドルール | 状態 |
|------------------|------|------------------|------|
| ✅ testcasedb-alb-sg | ALB用 | HTTPS:443 ← プロキシIP | ☐ |
| ✅ testcasedb-ecs-sg | ECS用 | TCP:3000 ← ALB-SG | ☐ |
| ✅ testcasedb-rds-sg | RDS用 | PostgreSQL:5432 ← ECS-SG, Batch-SG | ☐ |
| ✅ testcasedb-batch-sg | Batch用 | なし | ☐ |
