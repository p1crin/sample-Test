# AWS デプロイメント手順書（インターネット公開版・IP制限あり）
...
| 許可 IP レンジ | 203.0.113.0/24, 198.51.100.0/24 | WAF、SG でのアクセス制限 |
| 管理者用 IP | 203.0.113.100/32 | 緊急アクセス用 |

---

## 2. 全体構成概要

### 2.1 インターネット公開版アーキテクチャ
...
| アクセス制御 | **WAF IP制限 + SG** | 多層防御 |
| SSL 証明書 | **ACM パブリック証明書** | Let's Encrypt 互換 |
| WAF | **必須** | IP制限、レート制限、脅威検出 |
...
| 説明 | Security group for Internet-facing ALB with IP restriction |
| VPC | `testcasedb-vpc` |
...
| タイプ | ポート | ソース | 説明 |
|--------|--------|--------|------|
| HTTPS | 443 | 203.0.113.0/24 | オフィスネットワーク |
| HTTPS | 443 | 198.51.100.0/24 | 自宅・リモートワーク用 |
| HTTPS | 443 | 203.0.113.100/32 | 管理者専用IP（緊急時用） |
...
2. **動的 IP への対応**
   - 動的 IP のユーザーには VPN を提供
   - または認証強化（MFA 必須）で WAF IP 制限を緩和

3. **緊急時のアクセス**
   - 管理者用の緊急アクセス IP を別途確保
   - 一時的な IP 追加手順を文書化
