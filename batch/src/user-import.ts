...
import { ImportSummary, UserCsvRow, UserImportResult } from './types/user-import.types';
import { parseCsv, parseStatus, validateAllRows } from './utils/csv-parser';
...
const STRAGE_MODE = process.env.STORAGE_MODE || 's3';
...
function validateEnvironmentVariables(): void {
  ...
  } else if (STRAGE_MODE === 'local') {
    required.push('INPUT_FILE_PATH', 'OUTPUT_DIR_PATH');
  } else {
    throw new Error(`STORAGE_MODEは "s3" または "local" である必要があります（現在の値: ${STRAGE_MODE}）`);
  }
