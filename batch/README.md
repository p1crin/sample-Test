...
CSVファイルは以下のカラムを含む必要があります。**日本語ヘッダーと英語ヘッダーの両方に対応**しています。
...
| tags | タグ | - | セミコロン（;）区切りで複数指定可能 |
| status | ステータス | - | 「有効」または「無効」（デフォルト: 有効） |
...
| ID | ユーザID | - | 新規作成時は空、更新時は既存のID |
| 氏名 | 名前 | ✓ | |
| ID（メールアドレス） | メールアドレス | ✓ | 重複不可 |
| 権限 | ユーザロール | ✓ | 0: システム管理者, 1: テスト管理者, 2: 一般 |
| 部署 | 部署 | - | |
| 会社 | 会社 | - | |
| パスワード | パスワード | △ | 新規作成時は必須、更新時は変更する場合のみ指定 |
| タグ | タグ | - | セミコロン（;）区切りで複数指定可能 |
| ステータス | ステータス | - | 「有効」または「無効」（デフォルト: 有効） |

**対応するヘッダー名のバリエーション**:
- ID: `id`, `ID`, `ユーザID`
- 氏名: `name`, `名前`, `氏名`
- メールアドレス: `email`, `Email`, `メールアドレス`, `ID（メールアドレス）`
- 権限: `user_role`, `権限`, `ユーザロール`, `ロール`
- タグ: `tags`, `タグ`
- ステータス: `status`, `ステータス`, `状態`

### CSV サンプル

#### 英語ヘッダー版
```csv
id,name,email,user_role,department,company,password,tags,status
,山田太郎,yamada@example.com,1,開発部,株式会社ABC,password123,開発;フロントエンド,有効
,佐藤花子,sato@example.com,2,品質保証部,株式会社ABC,securepass,QA;テスト,有効
1,鈴木一郎,suzuki@example.com,0,IT部,株式会社ABC,,管理者;インフラ,有効
```

#### 日本語ヘッダー版
```csv
ID,氏名,ID（メールアドレス）,権限,部署,会社,パスワード,タグ,ステータス
,山田太郎,yamada@example.com,1,開発部,株式会社ABC,password123,開発;フロントエンド,有効
,佐藤花子,sato@example.com,2,品質保証部,株式会社ABC,securepass,QA;テスト,有効
1,鈴木一郎,suzuki@example.com,0,IT部,株式会社ABC,,管理者;インフラ,無効
```

**タグの例**:
- 単一タグ: `開発`
- 複数タグ: `開発;フロントエンド;React`
- タグなし: 空欄

**ステータスの値**:
- 有効: `有効`, `active`, `0`, `false` (デフォルト)
- 無効: `無効`, `inactive`, `1`, `true`
...
3. **ヘッダー正規化**: 日本語ヘッダーを英語プロパティ名に変換
4. **事前バリデーション**: 全行をチェック
   - 必須項目チェック（名前、メール、ロール、新規時のパスワード）
   - 形式チェック（メールアドレス、ロール値、ユーザID、ステータス）
   - **エラーがあれば実行せずに終了**、エラー詳細を`tt_import_results`に記録
5. **トランザクション実行**: 全ユーザを一括処理
   - **新規ユーザ** (IDが空):
     - ユーザを新規作成
     - タグを作成・関連付け
     - ステータス（有効/無効）を設定
